...
function WPOSItems() {
    ...
    function addItemRow(qty, name, unit, wunit, taxid, reorderpoint, totalItems, sitemid, data, otherRelatedItemsId, totalStockLevel, hidden) {
        ...
        var row = $('<tr class="item_row"' + ' style="display: '+ (hidden? "none": "visible")+';">' +
            ...
            '<td><select onchange="WPOS.sales.updateSaleUnit($(this).val(), $(this).parent().parent());" style="max-width:50px;" class="itemtype form-control">' +getTypeSelectHTML(unit, wunit)+ '</select><input class="itemtypeval" type="hidden" value="0" /></td>' +
            '<td class="itemprice"><input onChange="WPOS.sales.updateSalesTotal();" style="max-width:75px;" type="text" class="itemunit form-control numpad" value="' + unit + '" /></td>' +
            '<td><select '+((!newItem && disabletax==true && taxid!=null)?"disabled":"")+' onChange="WPOS.sales.updateSalesTotal();" style="max-width:50px;" class="itemtax form-control">' +getTaxSelectHTML(taxid)+ '</select><input class="itemtaxval" type="hidden" value="0.00" /></td>' +
...
// Implemented new modal for aborting orders
    ...
    function getSaleObject(){
        ...
        var items = [];
        var taxtotals = {};
        var taxdata, itemdata, taxruleid, tempqty, numitems = 0, totalcost = 0, stockLevel, totalStockLevel, otherItemsId;
        var orders = {};
        var oldorders = {};
        var neworderid = null;
