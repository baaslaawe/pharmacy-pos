...
<div class="page-header">
    ...
    <button onclick="openAddStockDialog();" id="addbtn" class="btn btn-primary btn-sm pull-right"><i
                class="icon-pencil align-top bigger-125"></i>Add
    </button>
    <button class="btn btn-success btn-sm pull-right" style="margin-right: 10px;" onclick="exportStock();"><i
                class="icon-cloud-download align-top bigger-125"></i>Download Template
    </button>
    <button class="btn btn-success btn-sm pull-right" style="margin-right: 10px;" onclick="openImportDialog();"><i
                class="icon-cloud-upload align-top bigger-125"></i>Update Inventory
    </button>
    ...
    <div class="col-xs-12">
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <table id="stocktable" class="table table-striped table-bordered table-hover dt-responsive"
                       style="width:100%;">
                    <thead>
                    <tr>
...
<script type="text/javascript">
            ...
            "aoColumns": [
                ...
                {mData: "code"},
                {
                    mData: function (data, type, val) {
                        return (data.name == null ? "Unknown" : data.name)
                    }
                },
                {mData: "description"},
                {
                ...
                {
                    mData: function (data, type, val) {
                        return (data.categoryid !== '0' ? (categories.hasOwnProperty(data.categoryid) ? categories[data.categoryid].name : 'Unknown') : 'General');
                    }
                },
                {
                    mData: function (data, type, val) {
                        return '<div class="action-buttons"><a class="green" onclick="openEditStockDialog(' + data.id + ');"><i class="icon-pencil bigger-130"></i></a><a class="blue" onclick="openAddToStockDialog(' + data.id + ');"><i class="icon-plus bigger-130"></i></a><a class="blue" onclick="openTransferStockDialog(' + data.id + ')"><i class="icon-arrow-right bigger-130"></i></a><a class="red" onclick="getStockHistory(' + data.id + ', ' + data.locationid + ');"><i class="icon-time bigger-130"></i></a><a class="red" onclick="deleteStockItem(' + data.id + ');"><i class="icon-trash bigger-130"></i></a></div>';
                    }, "bSortable": false
                }
            ],
            "columns": [
    ...
    function openEditStockDialog(id) {
        ...
        $("#setstockcost").val(isAdmin ? item.cost:"****");
    ...
    function exportStock() {
        ...
        var csv = WPOS.data2CSV(
            ...
            ['code', 'name', 'description',
                {
                    key: 'locationid', func: function (value) {
                ...
                'cost', 'price', 'wprice', 'stocklevel', 'reorderpoint', 'supplier', 'inventoryNo', 'expiryDate', 'taxname',
                {
                    key: 'categoryid', func: function (value) {
                        return categories.hasOwnProperty(value) ? categories[value].name : 'GENERAL';
                    }
                }
            ],
            data
