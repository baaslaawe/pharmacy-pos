                    ...
                    <div class="widget-body">
                        <div class="widget-main no-padding">
                            <table class="table table-bordered table-striped">
                                ...
                                <tr id="nosalesrow">
                                    <td colspan="5" style="text-align: center;"><strong>No sales data for today</strong>
                                    </td>
                                </tr>
                                </tbody>
                            </table>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <div style="text-align: center;">
                                                <button class="btn btn-primary btn-sm" onclick="sendMessage();">Send
                                                </button>
                                                <button class="btn btn-danger btn-sm" onclick="sendReset();">Reset
                                                    Terminal
                                                </button>
                                            </div>
                                        </td>
...
<script type="text/javascript">
    ...
    function sendMessage() {
        if (Object.keys(onlinedev).length <= 1) {
            swal({
                type: 'error',
                title: 'Oops...',
                text: 'There are no devices online to message'
            });
    ...
    function sendReset() {
        if (Object.keys(onlinedev).length <= 1) {
            swal({
                type: 'error',
                title: 'Oops...',
                text: 'There are no devices online to reset'
            });
        ...
        if (result !== false) {
            swal({
                type: 'success',
                title: 'Success.',
                text: 'The reset request has been sent to the selected devices.'
            });
        }
        // hide loader
        WPOS.util.hideLoader();
    ...
    function reloadGraph() {
        ...
        // fetch the data
        var data = WPOS.sendJsonData("graph/general", JSON.stringify({
            "stime": stime,
            "etime": etime,
            "interval": 1800000
    ...
    $(function () {
        ...
        var data = WPOS.sendJsonData("multi", JSON.stringify({
            "sales/get": {stime: stoday, etime: etime},
            "stats/general": {"stime": stoday, "etime": etime},
            "graph/general": {"stime": stime, "etime": etime, "interval": 1800000}
        }));
        if (data !== false) {
