...
class TransactionsModel extends DbConfig
{
    ...
    public function getTotals($stime, $etime, $status=null, $statparity=null, $includeorders=false, $ttype=null){
        ...
        $placeholders = [":stime"=>$stime, ":etime"=>$etime];
        $sql = "SELECT *, COALESCE(SUM(total), 0) as stotal, COALESCE(SUM(cost), 0) as ctotal, COALESCE(SUM(discount), 0) as cdiscount, COUNT(id) as snum, COALESCE(GROUP_CONCAT(ref SEPARATOR ','),'') as refs FROM sales WHERE (processdt>= :stime AND processdt<= :etime)";
