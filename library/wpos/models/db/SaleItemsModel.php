...
class SaleItemsModel extends DbConfig
{
    ...
    public function getStoredItemTotalsCategory($stime, $etime, $group = 0, $novoids = true, $ttype=null){
        ...
        $sql = "SELECT ".($group>0?'store.'.$groupcol.' AS groupid, p.name AS name':'i.storeditemid AS groupid, i.name AS name').", COALESCE(SUM(i.qty), 0) AS itemnum, COALESCE(SUM(i.price-s.discount), 0) AS itemtotal, COALESCE(SUM(s.discount), 0) AS discounttotal, COALESCE(SUM(i.tax_total), 0) AS taxtotal, COALESCE(SUM(i.refundqty), 0) AS refnum, COALESCE(SUM(i.unit*i.refundqty), 0) AS reftotal, COALESCE(GROUP_CONCAT(DISTINCT s.ref SEPARATOR ','),'') as refs";
        $sql.= ' FROM sale_items AS i LEFT JOIN sales AS s ON i.saleid=s.id'.($group>0 ? ' LEFT JOIN stock_items AS si ON i.storeditemid=si.id  LEFT JOIN stock_inventory AS s_inv ON si.stockinventoryid=s_inv.id  LEFT JOIN stored_items AS store ON store.id=s_inv.storeditemid LEFT JOIN '.$grouptable.' AS p ON store.'.$groupcol.'=p.id' : '').' WHERE (s.processdt>= :stime AND s.processdt<= :etime) '.($novoids?'AND s.status!=3':'');
        $placeholders = [":stime"=>$stime, ":etime"=>$etime];
