...
class QRimage
{
    ...
    private static function image($frame, $pixelPerPoint = 4, $outerFrame = 8, $back_color = 0xFFFFFF, $fore_color = 0x000000)
    {
        $h = count($frame);
        $w = strlen($frame[0]);
        // pad image using different outerframe value for width
        $outerFrameWidth = 20;
        //$outerFrameWidth = $outerFrame;

        $imgW = $w + 2 * $outerFrameWidth;
        $imgH = $h + 2 * $outerFrame;
        ...
        for ($y = 0; $y < $h; $y++) {
            for ($x = 0; $x < $w; $x++) {
                if ($frame[$y][$x] == '1') {
                    ImageSetPixel($base_image, $x + $outerFrameWidth, $y + $outerFrame, $col[1]);
                }
            }
        }
    ...
    {
        $image = self::image($frame, $pixelPerPoint, $outerFrame, null, null);
