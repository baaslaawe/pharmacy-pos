    ...
    <div>
        <ul class="list-unstyled spaced">
            ...
            <li>
                ...
                Folder Permissions: App Root
                / <?php echo($deps['permissions_root'] ? "is writable" : "must be writable") ?>
            </li>
            <li>
                <i class="icon icon-large icon-check <?php echo($deps['permissions_files'] ? "green" : "red"); ?>"></i>
                File Permissions: Application
                files <?php echo($deps['permissions_files'] ? "are not writable" : "must not be writable") ?>
            </li>
            <li>
                <i class="icon icon-large icon-check <?php echo($deps['permissions_docs'] ? "green" : "red"); ?>"></i>
                File Permissions: Documents
                (/docs/) <?php echo($deps['permissions_docs'] ? " are writable" : "files must be writable") ?>
            </li>
            <li>
                <i class="icon icon-large icon-check <?php echo($deps['permissions_config'] ? "green" : "red"); ?>"></i>
                File Permissions: Config
                files <?php echo($deps['permissions_config'] ? " are writable" : " .config.json && .dbconfig.json (/library/wpos/) must be writable") ?>
            </li>
            <li>
            ...
            if (!$deps['all']) {
                ?>
                <li>
                    <label><input type="checkbox" data-reqs-met="<?php echo($deps['all']); ?>"
                                  onchange="$('#next-button').prop('disabled', ($(this).is(':checked')?false:true))"/>
        ...
        <div style="height: 40px;">
            ...
            <form method="post">
                ...
                <button id="next-button" type="submit"
                        class="pull-right btn btn-primary" <?php echo($deps['all'] ? "" : "disabled='disabled'"); ?> ><?php echo($curversion > 0 ? "Upgrade" : "Next"); ?></button>
            </form>
        </div>
    </div>
</div>
